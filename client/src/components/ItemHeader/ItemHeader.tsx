import './ItemHeader.css';

export interface ItemHeaderProps {
  title: string;
  price: number;
  createdAt: string;
  updatedAt: string;
  onEditClick?: () => void;
}

const formatDate = (value: string) =>
  new Intl.DateTimeFormat('ru-RU', {
    day: 'numeric',
    month: 'long',
    hour: '2-digit',
    minute: '2-digit',
  }).format(new Date(value));

export const ItemHeader = ({
  title,
  price,
  createdAt,
  updatedAt,
  onEditClick,
}: ItemHeaderProps) => {
  const wasEdited =
    new Date(updatedAt).getTime() > new Date(createdAt).getTime();
  return (
    <div className="flex item-header">
      <div className="item-header__info">
        <h1 className="item-header__title">{title}</h1>

        <button
          type="button"
          className="btn-reset flex item-header__edit-button"
          onClick={onEditClick}
        >
          Редактировать
          <svg
            width="17"
            height="17"
            viewBox="0 0 17 17"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M2.92701 12.8571C2.96719 12.8571 3.00737 12.8531 3.04755 12.8471L6.42656 12.2545C6.46674 12.2464 6.50491 12.2283 6.53304 12.1982L15.0489 3.68237C15.0675 3.66378 15.0823 3.64171 15.0924 3.6174C15.1024 3.5931 15.1076 3.56705 15.1076 3.54074C15.1076 3.51443 15.1024 3.48837 15.0924 3.46407C15.0823 3.43977 15.0675 3.41769 15.0489 3.39911L11.71 0.058259C11.6719 0.0200893 11.6217 0 11.5674 0C11.5132 0 11.4629 0.0200893 11.4248 0.058259L2.90893 8.57411C2.87879 8.60424 2.86071 8.6404 2.85268 8.68058L2.26004 12.0596C2.2405 12.1672 2.24748 12.278 2.28039 12.3823C2.31329 12.4866 2.37113 12.5813 2.44888 12.6583C2.58147 12.7868 2.74821 12.8571 2.92701 12.8571V12.8571ZM4.28103 9.35357L11.5674 2.0692L13.04 3.54174L5.75357 10.8261L3.96763 11.1415L4.28103 9.35357V9.35357ZM15.4286 14.5446H0.642857C0.287277 14.5446 0 14.8319 0 15.1875V15.9107C0 15.9991 0.0723214 16.0714 0.160714 16.0714H15.9107C15.9991 16.0714 16.0714 15.9991 16.0714 15.9107V15.1875C16.0714 14.8319 15.7842 14.5446 15.4286 14.5446Z"
              fill="white"
            />
          </svg>
        </button>
      </div>

      <div className="item-header__meta">
        <p className="item-header__price">{price.toLocaleString('ru-RU')} ₽</p>

        <div className="item-header__dates">
          <p className="item-header__date">
            Опубликовано: {formatDate(createdAt)}
          </p>
          {wasEdited && (
            <p className="item-header__date">
              Отредактировано: {formatDate(updatedAt)}
            </p>
          )}
        </div>
      </div>
    </div>
  );
};
