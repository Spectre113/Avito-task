import './ItemRevisionNotice.css';

export interface ItemRevisionNoticeProps {
  fields: string[];
}

export const ItemRevisionNotice = ({ fields }: ItemRevisionNoticeProps) => {
  if (!fields.length) {
    return null;
  }

  return (
    <div className="item-revision-notice">
      <div className="flex item-revision-notice__header">
        <span className="item-revision-notice__icon">
          <svg
            width="16"
            height="16"
            viewBox="0 0 16 16"
        ...
        <h2 className="item-revision-notice__title">Требуются доработки</h2>
      </div>
      <div className="item-revision-notice__block">
        <p className="item-revision-notice__text">
          У объявления не заполнены поля:
        </p>
        <ul className="list-reset item-revision-notice__list">
          {fields.map((field) => (
            <li key={field} className="item-revision-notice__item">
              {field}
            </li>
          ))}
        </ul>
